...
pub fn parse_all<'linespan>(ast_output: &mut ASTOutput<'linespan>, cst_output: &'linespan CSTOutput<'linespan>) -> bool {
    ...
    loop {
        match curr_ast_block.parse_block(&mut cn_iter) {
            ...
            None => {
                ast_output.output.push(curr_ast_block);
                let Some(prev) = prev_indent_lvs.last() else { break };
                if *prev == 0 { break }
...
impl<'linespan> ASTBlock<'linespan> {
    ...
    fn parse_block<I: Iterator<Item = &'linespan CSTNode<'linespan>>>(&mut self, cn_iter: &mut Peekable<I>) -> Option<ASTBlock<'linespan>> {
        loop {
            ...
            match cnode {
                ...
                => {
                    ...
                    if self.indent_lv < *indent_sz { break }
                    ...
                    let Some(__num) = self.parse_expr(*(_num.clone())) else { continue };
                ...
                _ => {
                    let Some(expr) = self.parse_expr((*cnode).clone()) else { continue };
            ...
            match rval {
                ...
                None => if err_count < self.errs.len() || block_count < self.block.len() { continue }
                	else { break }
            }
        }
    ...
    fn parse_print(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        if *ptype != PrintType::Normal {
            ...
            let Some(__spcl_x_pos) = self.parse_expr(*_spcl_x_pos) else { return None };
            ...
            *spcl_x = Some(Box::new(__spcl_x_pos));
            ...
            let Some(__spcl_y_pos) = self.parse_expr(*_spcl_y_pos) else { return None };
    ...
    fn parse_for(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        let Some(_start) = start else {
            self.errs.push(ASTReport::MissingXInY{
                x: "Starting number".to_string(),
                y: _cnode,
            });
            return None;
        };

        let Some(__start) = self.parse_expr(*_start) else { return None };
        ...
        let Some(_end) = end else {
            self.errs.push(ASTReport::MissingXInY{
                x: "Ending number".to_string(),
                y: _cnode,
            });
            return None;
        };

        let Some(__end) = self.parse_expr(*_end) else { return None };
    ...
    fn parse_play(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        let Some(_sound) = sound else {
            self.errs.push(ASTReport::MissingXInY{
                x: "Sound Name".to_string(),
                y: _cnode
            });
            return None;
        };

        let Some(__sound) = self.parse_expr(*_sound) else { return None };
        ...
        let Some(__pitch) = self.parse_expr(*_pitch) else { return None };
    ...
    fn parse_brew(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        let Some(_ingr1) = ingr1 else {
            self.errs.push(ASTReport::MissingXInY{
                x: "Ingredient 1".to_string(),
                y: _cnode.clone(),
            });
            let Some(_plus) = plus else { return None };
            *op = ASTOpType::default().translate_ltok(_plus);

            let Some(_ingr2) = ingr2 else {
                self.errs.push(ASTReport::MissingXInY{
                    x: "Ingredient 2".to_string(),
                    y: _cnode
                });
                return None;
            };

            let Some(__ingr2) = self.parse_expr(*_ingr2) else { return None };
        ...
        *lhs = Some(Box::new(__ingr1.clone()));
        ...
        let Some(__plus) = ASTOpType::default().translate_ltok(_plus) else { return None };
        *op = Some(__plus);
    ...
    fn parse_disable(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        let Some(__opt) = self.parse_expr(*_opt) else { return None };
    ...
    fn parse_equip(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        for node in tool_name {
            let Some(expr) = self.parse_expr(node) else { break };
    ...
    fn parse_var_def(&mut self, cnode: CSTNode<'linespan>) -> Option<ASTBlock<'linespan>> {
        ...
        let Some(_name) = self.parse_expr(*name) else { return None };
